...
def make_clip_loss_fn(root, args):
    clip_size = gs.models["clip"].visual.input_resolution # for openslip: clip_model.visual.image_size
    ...
    def parse_clip_prompts(clip_prompt):
        ...
        for prompt in clip_prompt:
            ...
            target_embeds.append(gs.models["clip"].encode_text(clip.tokenize(txt).to(root.device)).float())
    ...
    def clip_loss_fn(x, sigma, **kwargs):
        ...
        image_embeds = gs.models["clip"].encode_image(clip_in).float()

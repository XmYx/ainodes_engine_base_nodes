...
class Kandinsky2_2:
    ...
    def generate_text2img(
        ...
        img_emb = self.prior(prompt=prompt, num_inference_steps=int(prior_steps),
                             num_images_per_prompt=batch_size, guidance_scale=prior_cf_scale,
                             negative_prompt=negative_prior_prompt)
        negative_emb = self.prior(prompt=negative_decoder_prompt, num_inference_steps=int(prior_steps),
                                  num_images_per_prompt=batch_size, guidance_scale=prior_cf_scale)
        ...
        images = self.decoder(image_embeds=img_emb.image_embeds, negative_image_embeds=negative_emb,
                              num_inference_steps=num_steps, height=h,
                              width=w, guidance_scale=guidance_scale).images
        return images
    ...
    def generate_img2img(
        ...
        img_emb = self.prior(prompt=prompt, num_inference_steps=int(prior_steps),
                             num_images_per_prompt=batch_size, guidance_scale=prior_cf_scale,
                             negative_prompt=negative_prior_prompt)
        negative_emb = self.prior(prompt=negative_prior_prompt, num_inference_steps=int(prior_steps),
                                  num_images_per_prompt=batch_size, guidance_scale=prior_cf_scale)
        if negative_decoder_prompt == "":
            negative_emb = negative_emb.negative_image_embeds
        else:
        ...
        images = self.decoder(image_embeds=img_emb.image_embeds, negative_image_embeds=negative_emb,
                              num_inference_steps=num_steps, height=h,
                              width=w, guidance_scale=guidance_scale,
